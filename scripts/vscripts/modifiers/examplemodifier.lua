...
function examplemodifier:GetModifierMoveSpeedOverride(event)
	ms = self.ms or 300
	return (ms<=295) and ms*1.1 or ms
end

function examplemodifier:OnCreated(event)
	self.ms = self:GetParent():GetBaseMoveSpeed()
...
function examplemodifier:OnTeleporting(event)
	-- for k,v in pairs(event) do print("OnDeath",k,v) end -- find out what event.__ to use
	if IsClient() then return end
	self:GetParent():AddItemByName("item_tpscroll")
